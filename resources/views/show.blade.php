    ...
    <main>
            ...
            <div class="max-w-screen-xl px-4 mx-auto lg:px-12 w-full">
                ...
                <div class="flex flex-col items-center justify-between p-4 space-y-3 md:flex-row md:space-y-0 md:space-x-4">
                    ...
                    <form class="flex items-center">
                        ...
                        <input type="search" id="search" name="search" value="{{ $search ?? null}}" class="block w-full p-2 pl-10 text-sm text-gray-900 border border-gray-300 rounded-lg bg-gray-50 focus:ring-primary-500 focus:border-primary-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="Search">
                        </div>
        ...
        <section class="bg-gray-50 dark:bg-gray-900 p-3 sm:p-5">
            <div class="mx-auto max-w-screen-xl px-4 lg:px-12">
                ...
                @endif
                @if ($swimmings->isEmpty() && $footballs->isEmpty() && $badmintons->isEmpty())
                    <div class="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden">
                        <div class="p-4 sm:p-5">
                            <h2 class="text-lg font-semibold text-gray-900 dark:text-white">No data available</h2>
                        </div>
                    </div>
                @endif
                <div class="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden">
                    <div class="overflow-x-auto">
                        ...
                        <table class="mb-10 w-full text-sm text-left text-gray-500 dark:text-gray-400">
                            ...
                            <tbody>
                                ...
                                <tr class="border-b dark:border-gray-700">
                                    <th scope="row" class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">{{ $swimming->name }}</th>
                                    <td class="px-4 py-3">{{ $swimming->category->name }}</td>
                                    <td class="px-4 py-3">{{ $swimming->distance }}</td>
                                    <td class="px-4 py-3">{{ $swimming->stroke }}</td>
                                    <td class="px-4 py-3 flex items-center justify-end">
                                        <a href="/dashboard/swimming/{{ $swimming->id }}/edit">
                                        ...
                                        <form action="/dashboard/swimming/{{ $swimming->id }}" method="post">
                                            @method('delete')
                                            @csrf
                        ...
                        <table class="mb-10 w-full text-sm text-left text-gray-500 dark:text-gray-400">
                            ...
                            <tbody>
                                ...
                                <tr class="border-b dark:border-gray-700">
                                    <th scope="row" class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">{{ $football->name }}</th>
                                    <td class="px-4 py-3">{{ $football->category->name }}</td>
                                    <td class="px-4 py-3">{{ $football->category_umur }}</td>
                                    <td class="px-4 py-3 flex items-center justify-end">
                                        <a href="/dashboard/football/{{ $football->id }}/edit">
                                            <button type="button" class="text-white bg-yellow-400 hover:bg-yellow-500 focus:outline-none focus:ring-4 focus:ring-yellow-300 font-medium rounded-full text-sm px-5 py-2.5 text-center me-2 mb-2 dark:focus:ring-yellow-900">Edit</button>
                                        </a>
                                        <form action="/dashboard/football/{{ $football->id }}" method="post">
                                            @method('delete')
                                            @csrf
                                            <button type="submit" onclick="return confirm('Are you sure?')" class="text-white bg-red-700 hover:bg-red-800 focus:outline-none focus:ring-4 focus:ring-red-300 font-medium rounded-full text-sm px-5 py-2.5 text-center me-2 mb-2 dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-red-900">Delete</button>
                                        </form>
                        ...
                        <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                            ...
                            <tbody>
                                ...
                                <tr class="border-b dark:border-gray-700">
                                    <th scope="row" class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">{{ $badminton->name }}</th>
                                    <td class="px-4 py-3">{{ $badminton->category->name }}</td>
                                    <td class="px-4 py-3">{{ $badminton->category_kelas }}</td>
                                    <td class="px-4 py-3 flex items-center justify-end">
                                        <a href="/dashboard/badminton/{{ $badminton->id }}/edit">
                                            <button type="button" class="text-white bg-yellow-400 hover:bg-yellow-500 focus:outline-none focus:ring-4 focus:ring-yellow-300 font-medium rounded-full text-sm px-5 py-2.5 text-center me-2 mb-2 dark:focus:ring-yellow-900">Edit</button>
                                        </a>
                                        <form action="/dashboard/badminton/{{ $badminton->id }}" method="post">
                                            @method('delete')
                                            @csrf
                                            <button type="submit" onclick="return confirm('Are you sure?')" class="text-white bg-red-700 hover:bg-red-800 focus:outline-none focus:ring-4 focus:ring-red-300 font-medium rounded-full text-sm px-5 py-2.5 text-center me-2 mb-2 dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-red-900">Delete</button>
                                        </form>
